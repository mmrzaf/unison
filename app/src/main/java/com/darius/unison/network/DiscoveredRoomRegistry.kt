...
/**
 * De-duplicates rooms discovered during one user-triggered browse window. The registry is cleared
 * before every manual scan.
 */
class DiscoveredRoomRegistry {
    private val roomsByService = linkedMapOf<String, DiscoveredRoom>()

    @Synchronized fun clear() = roomsByService.clear()

    /** Returns true only when the visible room list changed. */
    @Synchronized
    fun found(room: DiscoveredRoom): Boolean {
        var changed = false
        val duplicateServices =
            roomsByService
    ...
    @Synchronized
    fun rooms(): List<DiscoveredRoom> =
        roomsByService.values.sortedWith { left, right ->
