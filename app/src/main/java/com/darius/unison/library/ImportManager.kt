...
class ImportManager(
    ...
    suspend fun importM3u(
        ...
        withContext(Dispatchers.IO) {
            ...
            val parsed =
                context.contentResolver.openInputStream(uri)?.use { input ->
                    ...
                    while (true) {
                        ...
                        if (read == 0) continue
                        totalBytes += read
                        require(totalBytes <= M3uCodec.MAX_FILE_BYTES) {
                            "M3U playlist is too large"
                        }
            ...
            parsed.entries.forEachIndexed { index, entry ->
                ...
                when (
                    val resolution =
                        resolveEntry(entry, baseDirectory, treeIndex, manualSelections[index])
                ) {
                    is EntryResolution.Resolved ->
                        resolved += M3uResolvedEntry(index, entry, resolution.track)
                    is EntryResolution.Ambiguous ->
                        ambiguous += M3uAmbiguousEntry(index, entry, resolution.candidates)
                    is EntryResolution.Unresolved ->
                        unresolved += M3uUnresolvedEntry(index, entry, resolution.reason)
                }
                report(
                    stage = LibraryImportStage.RESOLVING_ENTRIES,
                    documentsScanned = indexedDocuments,
            ...
            val orderedTracks =
                resolved.sortedBy(M3uResolvedEntry::entryIndex).map { it.track.trackId }
            val playlistName =
                uri.lastPathSegment
                    ?.substringAfterLast('/')
                    ?.substringBeforeLast('.')
                    ?.trim()
                    ?.takeIf(String::isNotEmpty) ?: "Imported playlist"
            val playlistId =
    ...
    private suspend fun resolveEntry(
        ...
        manualSelection?.let { selected ->
            trackRepository.get(selected)?.let {
                return EntryResolution.Resolved(it)
            }
        }
        ...
        when (parsed?.scheme?.lowercase(Locale.ROOT)) {
            "content" ->
                suspendResult { trackRepository.importUri(parsed) }
                    .getOrNull()
                    ?.let {
                        return EntryResolution.Resolved(it)
                    }
            ...
            "file" ->
                safeFileFromReference(parsed.path.orEmpty(), baseDirectory)?.let { file ->
                    suspendResult { trackRepository.importUri(Uri.fromFile(file)) }
                        .getOrNull()
                        ?.let {
                            return EntryResolution.Resolved(it)
                        }
        ...
        val fileName =
            when (pathDecision) {
                ...
                is PlaylistPathPolicy.Decision.Rejected ->
                    reference.replace('\\', '/').substringAfterLast('/').trim()
            }
        val title =
            entry.displayTitle?.trim()?.ifBlank { null } ?: fileName.substringBeforeLast('.')
        val candidates = trackRepository.findReferenceCandidates(fileName, title)
        val decision =
            PlaylistMatchPolicy.decide(
                fileName = fileName,
        ...
        return when (decision) {
            is PlaylistMatchPolicy.Decision.Unique ->
                candidates
            ...
            PlaylistMatchPolicy.Decision.Missing ->
                EntryResolution.Unresolved("Playlist track is unavailable")
        }
    }
    ...
    private suspend fun buildTreeIndex(
        ...
        val root =
            DocumentFile.fromTreeUri(context, treeUri)
                ?: error("Unable to read selected music folder")
        ...
        while (queue.isNotEmpty() && visited < MAX_INDEXED_DOCUMENTS) {
            ...
            for (child in children) {
                ...
                val relativePath =
                    if (current.relativePath.isEmpty()) name else "${current.relativePath}/$name"
                if (child.isDirectory) {
                    queue.add(IndexedDirectory(child, relativePath, current.depth + 1))
                } else if (child.isFile && child.canRead()) {
                    val normalized =
                        (PlaylistPathPolicy.evaluate(relativePath)
                                as? PlaylistPathPolicy.Decision.Valid)
                            ?.normalizedPath ?: continue
                    byPath.getOrPut(normalized) { mutableListOf() }.add(child)
                    byName.getOrPut(name.lowercase(Locale.ROOT)) { mutableListOf() }.add(child)
                }
    ...
    private class TreeIndex(
        ...
        fun resolve(rawReference: String): TreeResolution {
            ...
            val valid =
                decision as? PlaylistPathPolicy.Decision.Valid
            ...
            when (
                ...
                is PlaylistTreeMatchPolicy.Decision.Found ->
                    return TreeResolution.Found(exact.value)
                is PlaylistTreeMatchPolicy.Decision.Ambiguous ->
                    return TreeResolution.Ambiguous(exact.count)
            ...
            when (
                val suffixMatch =
                    PlaylistTreeMatchPolicy.decide(suffixMatches, identity = { it.uri })
            ) {
                is PlaylistTreeMatchPolicy.Decision.Found ->
                    return TreeResolution.Found(suffixMatch.value)
                is PlaylistTreeMatchPolicy.Decision.Ambiguous ->
                    return TreeResolution.Ambiguous(suffixMatch.count)
                PlaylistTreeMatchPolicy.Decision.Missing -> Unit
            }
            ...
            return when (
                ...
                is PlaylistTreeMatchPolicy.Decision.Ambiguous ->
                    TreeResolution.Ambiguous(nameMatch.count)
    ...
    private fun Throwable.toUserMessage(): String =
        when {
            ...
            message?.contains("larger than", ignoreCase = true) == true ->
            ...
            message?.contains("not recognized as audio", ignoreCase = true) == true ->
                "This file is not supported audio"
            message?.contains("open", ignoreCase = true) == true ->
                "Unison could not open this file"
