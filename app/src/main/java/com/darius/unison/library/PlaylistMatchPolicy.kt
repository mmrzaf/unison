...
object PlaylistMatchPolicy {
    ...
    fun decide(
        ...
        val initial =
            when {
                ...
                normalizedTitle != null ->
                    candidates.filter { it.title.normalized() == normalizedTitle }
                else -> emptyList()
            }
        ...
        if (durationSeconds != null) {
            ...
            val durationMatches = narrowed.filter { candidate ->
                candidate.durationMs > 0L &&
                    abs(candidate.durationMs - expectedMs) <= DURATION_TOLERANCE_MS
            }
        ...
        return when (distinct.size) {
            ...
            1 -> Decision.Unique(distinct.single())
                ...
                Decision.Ambiguous(
                    distinct.sortedWith(
                        compareBy({ it.title.orEmpty() }, { it.fileName.orEmpty() }, { it.trackId })
                    )
                )
        }
    }
    ...
    private fun String?.normalized(): String? =
        this?.filterNot { it.isISOControl() }?.trim()?.lowercase(Locale.ROOT)?.ifBlank { null }
