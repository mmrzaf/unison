...
class TrackRepository(
    ...
    val temporaryTrackIds: Flow<Set<TrackId>> =
        database.trackSourceDao().observeTemporaryTrackIds().map { ids ->
            ids.mapTo(mutableSetOf(), ::TrackId)
        }
    val storageSummary: Flow<StorageSummary> =
        combine(
    ...
    suspend fun get(trackId: TrackId): TrackDescriptor? =
        database.trackDao().get(trackId.value)?.toDescriptor()
    ...
    suspend fun getMany(trackIds: List<TrackId>): List<TrackDescriptor> {
        ...
        val byId = entities.associate { TrackId(it.trackId) to it.toDescriptor() }
    ...
    suspend fun hasVerifiedSource(trackId: TrackId): Boolean {
        ...
        return verifiedManagedFile(trackId, expectedSize) != null ||
            bestReadableUri(trackId) != null
    }
     ...
     */
    suspend fun exportReference(trackId: TrackId): String? =
    ...
    suspend fun bestReadableUri(trackId: TrackId): Uri? =
        withContext(Dispatchers.IO) {
            val expectedSize = database.trackDao().get(trackId.value)?.sizeBytes
            ...
            database.trackSourceDao().getForTrack(trackId.value).firstNotNullOfOrNull { source ->
                when {
                    source.managedRelativePath != null ->
                        verifiedManagedFile(trackId, expectedSize)?.let(Uri::fromFile)
    ...
    /**
     * Imports audio into Unison's app-owned content-addressed store. File pickers, the Android
     * share sheet, M3U resolution, playlists, and room additions all use this same path so playback
     * never depends on a short-lived provider permission or a file that can move underneath the
     * room.
     */
        ...
        withContext(Dispatchers.IO) {
            ...
            while (true) {
                val page =
                    database
                        .trackSourceDao()
                        .temporaryTrackIdsAfter(afterTrackId, TEMPORARY_DELETE_BATCH_SIZE)
                if (page.isEmpty()) break
                afterTrackId = page.last()
                val candidates = page.filterNot(excluded::contains)
                if (candidates.isEmpty()) continue

                val managedFilesToDelete = mutableListOf<TrackId>()
                database.withTransaction {
                    val sourceDao = database.trackSourceDao()
                    sourceDao.deleteTemporaryForTracks(candidates)
                    val remaining = sourceDao.remainingTrackIds(candidates).toHashSet()
                    val remainingManaged =
                        sourceDao.remainingManagedTrackIds(candidates).toHashSet()
                    val tracksWithoutSources = candidates.filterNot(remaining::contains)
                    if (tracksWithoutSources.isNotEmpty())
                        database.trackDao().deleteMany(tracksWithoutSources)
                    candidates
    ...
    private fun strongerRetention(
        existing: RetentionPolicy?,
        requested: RetentionPolicy,
    ): RetentionPolicy =
        when {
            existing == RetentionPolicy.KEEP_IN_LIBRARY ||
                requested == RetentionPolicy.KEEP_IN_LIBRARY -> RetentionPolicy.KEEP_IN_LIBRARY
            existing == RetentionPolicy.TEMPORARY_24_HOURS ||
                requested == RetentionPolicy.TEMPORARY_24_HOURS ->
                RetentionPolicy.TEMPORARY_24_HOURS
    ...
    private fun String?.cleanText(maxLength: Int): String? =
        this?.filterNot { it.isISOControl() }?.trim()?.take(maxLength)?.ifBlank { null }
    ...
    companion object {
        ...
        private const val TEMPORARY_DELETE_BATCH_SIZE = 250
...
internal fun TrackEntity.toDescriptor() =
    TrackDescriptor(
...
internal fun TrackDescriptor.toEntity(now: Long) =
    TrackEntity(
        trackId = trackId.value,
        sizeBytes = sizeBytes,
        mimeType = mimeType,
