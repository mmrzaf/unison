...
/** Removes expired temporary sources without deleting active or otherwise referenced files. */
class CacheCleanupWorker(
    ...
    override suspend fun doWork(): Result = try {
        ...
        database.trackSourceDao().managedSources().forEach { source ->
            ...
            if (!store.hasVerified(trackId, source.expectedSizeOrNull(database))) {
