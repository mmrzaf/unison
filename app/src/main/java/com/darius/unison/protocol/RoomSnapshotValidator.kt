 ...
 * in the deterministic core harness. Callers that have encoded the snapshot should supply the
 * exact UTF-8 payload size through [SnapshotValidationContext.encodedSizeBytes].
 */
class RoomSnapshotValidator(
    ...
    fun validate(
        ...
        if (!snapshot.term.coordinatorPeerId.isValidPeerId()) reject("coordinator", "Invalid coordinator identity")
        ...
        context.expectedCoordinatorPeerId?.let {
            if (snapshot.term.coordinatorPeerId != it) {
                reject("wrong_coordinator", "Snapshot coordinator does not match the authenticated peer")
            }
        }
        context.minimumTerm?.let {
            if (snapshot.term.number < it) reject("old_term", "Snapshot term is older than the accepted term")
        }
        ...
        if (memberIds.distinct().size != memberIds.size) reject("duplicate_member", "Duplicate member identity")
        ...
        if (queueIds.distinct().size != queueIds.size) reject("duplicate_queue_item", "Duplicate queue item identity")
        val queueIdSet = queueIds.toSet()
        val memberIdSet = memberIds.toSet()
        snapshot.queue.forEachIndexed { index, item ->
            if (!item.queueItemId.isValidQueueItemId()) reject("queue_item_id", "Invalid queue item at index $index")
            ...
            if (!TRACK_ID_PATTERN.matches(track.trackId.value)) reject("track_id", "Invalid track hash")
            if (track.sizeBytes !in 1..MAX_TRACK_BYTES) reject("track_size", "Invalid track size")
            if (track.durationMs !in 0..MAX_TRACK_DURATION_MS) reject("track_duration", "Invalid track duration")
            if (!track.mimeType.isBoundedNullableText(MAX_MIME_LENGTH)) reject("mime", "Invalid MIME type")
            if (!track.title.isBoundedNullableText(MAX_METADATA_LENGTH)) reject("title", "Invalid track title")
            if (!track.artist.isBoundedNullableText(MAX_METADATA_LENGTH)) reject("artist", "Invalid track artist")
            if (!track.album.isBoundedNullableText(MAX_METADATA_LENGTH)) reject("album", "Invalid track album")
        ...
        if (snapshot.playback.positionAtTimestampMs < 0) reject("playback_position", "Playback position is negative")
        if (snapshot.playback.coordinatorTimestampNs < 0) reject("playback_timestamp", "Playback timestamp is negative")
        if (!snapshot.playback.playbackSpeed.isFinite() || snapshot.playback.playbackSpeed !in 0.5f..2.0f) {
            reject("playback_speed", "Playback speed is outside the protocol bounds")
        }

        if (snapshot.options.preloadCount !in 1..10) reject("preload", "Invalid preload count")
        ...
        return if (issues.isEmpty()) SnapshotValidationResult.Valid else SnapshotValidationResult.Invalid(issues)
    }
    ...
    private fun validateMember(
        ...
        if (!member.peerId.isValidPeerId()) reject("member_id", "Invalid member identity at index $index")
    ...
    private fun validateEndpoint(
        ...
        if (endpoint.peerId != expectedPeerId) reject("endpoint_peer", "Endpoint identity does not match its member")
        if (!endpoint.displayName.isBoundedText(1, MAX_DISPLAY_NAME_LENGTH)) {
            reject("endpoint_name", "Invalid endpoint display name")
        }
        if (!endpoint.hostAddress.isBoundedText(1, MAX_HOST_LENGTH)) reject("endpoint_host", "Invalid endpoint host")
        ...
        if (endpoint.lastSeenElapsedMs < 0) reject("endpoint_last_seen", "Invalid endpoint timestamp")
    }

    companion object {
        /** Leaves room for the envelope, discriminator fields, peer directory, and HMAC frame. */
        const val MAX_SNAPSHOT_PAYLOAD_BYTES: Int = 768 * 1024
