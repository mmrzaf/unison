...
class M3uResolutionPolicyTest {
    private fun track(id: Char, title: String) = TrackDescriptor(
    ...
    fun explicitChoicePreservesPlaylistOrder() {
        ...
        val ambiguity = M3uAmbiguousEntry(
            1,
            M3uEntry("second.mp3"),
            listOf(selected, track('d', "Other")),
        )
        val resolved = M3uResolutionPolicy.choose(listOf(third, first), ambiguity, selected.trackId)!!
        assertEquals(listOf("First", "Second", "Third"), resolved.map { it.track.title })
        assertEquals(listOf('a', 'b', 'c'), M3uResolutionPolicy.orderedTrackIds(resolved).map { it.value.first() })
    }
