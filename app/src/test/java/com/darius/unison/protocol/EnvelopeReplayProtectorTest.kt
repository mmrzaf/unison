...
class EnvelopeReplayProtectorTest {
    ...
    fun rejectsDuplicateMessageIds() {
        ...
        assertEquals(
            EnvelopeAcceptance.Accepted,
            guard.evaluate(peer, envelope(messageId = id), 3, 4),
        )
        assertEquals(
    ...
    fun reportsOrderedSequenceGap() {
        val guard = EnvelopeReplayProtector()
        val result =
            guard.evaluate(peer, envelope(sequence = 7), acceptedTerm = 3, lastAppliedSequence = 5)
...
private object TestSnapshots {
    fun basic(term: Long) =
        com.darius.unison.model.RoomSnapshot(
            roomId = "aaaaaaaa-aaaa-aaaa-aaaa-aaaaaaaaaaaa",
            roomName = "Room",
            term =
                com.darius.unison.model.CoordinatorTerm(
                    term,
